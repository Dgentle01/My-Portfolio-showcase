            ...
            {messages.length === 0 ? (
              ...
              <div className="grid gap-4">
                {messages.map((msg, index) => (
                  <Card key={msg.id || `msg-${index}`} className="bg-[#1a1a1a] border-white/5 hover:border-primary/40 transition-all group">
          ...
          <TabsContent value="projects" className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {projects.map((proj, index) => (
                <Card key={proj.id || `proj-${index}`} className="bg-[#1a1a1a] border-white/5 overflow-hidden group hover:border-primary/30 transition-all">
                ...
                <Card key={cat} className="bg-[#1a1a1a] border-white/5 flex flex-col group hover:border-primary/20 transition-all">
                  ...
                  <CardContent className="flex flex-wrap gap-2">
                    {skills.filter(s => s.category === cat).map((skill, index) => (
                      <Badge key={skill.id || `skill-${index}`} variant="secondary" className="bg-black text-white border border-white/10 flex items-center gap-2 pr-1 py-1 group/badge hover:border-primary/50 transition-colors">
                        {skill.name}
